...
function processLength(list, callback) {
  return callback(list.length, (num) => {
...
function processLastItem(stringList, callback) {
  ...
  return callback(last, (str) => str + str);
}
...
function processSum(numberList, callback) {
  let sum = numberList.reduce((acc, current) => {
    return acc += current
  }, 0);
...
function processProduct(num1, num2, callback) {
  ...
  return callback(numProduct, (num) => {
    return `${num} is a big number!`
  })
}
...
function processContains(item, list, callback) {
  let contents = list.includes(item);

  return callback(contents, (bool) => bool ? item : item)
...
function processDuplicateFree(list, callback) {
  let deduped = list.filter((item, index) => list.indexOf(item) === index);
  return callback(deduped, (arr) => arr);
}
...
function getFullNames(runners) {
  let racers = [];
  runners.forEach((runner) => racers.push(`${runner.last_name}, ${runner.first_name}`));
  return racers;
}
...
function firstNamesAllCaps(runners) {
  const powerTrip = runners.map((item) => item.first_name.toUpperCase());
  return powerTrip;
}
...
function tallyUpDonations(runners) {
  const donations = runners.reduce((acc, current) => {
